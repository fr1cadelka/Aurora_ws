...
public:
    LineFollower() : Node("line_follower") {
        ...
        this->declare_parameter<double>("roi_top_ratio", 0.4); // НОВЫЙ ПАРАМЕТР
        ...
        RCLCPP_INFO(this->get_logger(), "LineFollower (ORANGE LINES) started. Kp=%.2f, Ki=%.2f, Kd=%.2f, Speed=%.2f",
                    kp_, ki_, kd_, cruise_speed_);
        RCLCPP_INFO(this->get_logger(), "ROI top ratio: %.2f", roi_top_ratio_);
        RCLCPP_INFO(this->get_logger(), "Publishing to /line_follower/cmd_vel");
    }

private:
    int low_h1_ = 0, high_h1_ = 20, low_h2_ = 160, high_h2_ = 179;
    int low_s_ = 80, high_s_ = 255, low_v_ = 80, high_v_ = 255;
    int min_area_ = 500;
    int min_aspect_ = 30;
    int kp_track_ = 50, ki_track_ = 0, kd_track_ = 10;
    double roi_top_ratio_ = 0.4; // НОВАЯ ПЕРЕМЕННАЯ

    // НОВЫЙ МЕТОД - обрезаем верхнюю часть кадра
    ...
    void image_callback(const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
        ...
        // ----- ОБРЕЗАЕМ ROI -----
        cv::Mat frame_roi = get_roi(frame);
        ...
        cv::Mat hsv;
        cv::cvtColor(frame_roi, hsv, cv::COLOR_BGR2HSV);  // Работаем с обрезанным кадром
        ...
        if (both_found) {
            ...
            int frame_center_x = frame_roi.cols / 2;  // Используем ROI
        ...
        if (debug_) {
            cv::imshow("Original", frame_roi);  // Показываем обрезанный кадр
            cv::imshow("Mask", mask);
            ...
            cv::line(debug_frame, cv::Point(frame_roi.cols/2, 0), cv::Point(frame_roi.cols/2, frame_roi.rows), cv::Scalar(255,255,0), 2);
